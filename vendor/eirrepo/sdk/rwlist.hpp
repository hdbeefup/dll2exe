...
#define LIST_VALIDATE(item) ( FATAL_ASSERT( LIST_ISVALID( (item) ) ) )
...
#define LIST_INITNODE(link) ( (link).prev = nullptr, (link).next = nullptr )
#ifdef _DEBUG
...
template <typename listType>    inline void LIST_VALIDATE( listType& item )                     { return FATAL_ASSERT( LIST_ISVALID( item ) ); }
...
template <typename listType>    inline void LIST_INITNODE( listType& link )                     { link.prev = nullptr; link.next = nullptr; }
...
struct RwList
{
    ...
    inline void moveIntoBegin( RwList&& right )
    {
    ...
    inline void moveIntoEnd( RwList&& right )
    {
    ...
    inline RwList& operator = ( RwList&& right )
    {
